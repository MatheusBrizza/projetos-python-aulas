while True:
            operacao_teoria = input("Digite a operação desejada em Comandos (00= Teoria ,01 = Classes, 02 = Objetos,03 = Atributos, 04 = Métodos,05 = Herança 06 = Polimorfismo, 07 = Encapsulamento,08=Abstração,09=Modificadores de Acesso,11=Métodos Especiais): ")
            if operacao_teoria =="01":
                ...
                print("encapsulamento visa controlar o acesso a atributos e métodos de uma classe")
                ...
                continuar = input("Deseja printar o código a baixo?(s/n)")
                if continuar.lower()=="s":
                     class Produto:
                        ...
                        def calcular_desconto(self, porcentagem):
                            try:
                                if 0 <= porcentagem <= 100:
                                    ...
                                    raise ValueError("Porcentagem de desconto inválida! Deve estar entre 0 e 100.")
                            except ValueError as e:
                                print(f"Erro ao calcular desconto: {e}")
                                return self.__preco  
                     produto1 = Produto("123", "Camisa", 50, 10)
                     print(produto1.calcular_desconto(20))  

                     produto1.calcular_desconto(-50)  
                ...
                if continuar.lower() == "s":
                    ...
                    class Animal(ABC):
                    #Classe abstrata para representar animais.
                        ...
                        @abstractmethod  # Marca como método abstrato
                        def comer(self):
                            """
                            Método abstrato para representar a ação de comer.
                            Deve ser implementado em cada subclasse específica.
                            """
                            pass#é usado quando não há nenhuma instrução específica a ser executada

                        class Cachorro(Animal):
                        #Classe concreta para representar um cachorro.

                            def comer(self):
                                print(f"{self.nome} (o cachorro) está comendo ração.")

                        # Erro! Método abstrato não implementado na classe abstrata
                        # Animal.comer()

                        cachorro = Cachorro("late", "vira lata", 1)  # Instanciando uma classe concreta
                        cachorro.comer()  # Chamada do método implementado na subclasse
                ...
                print("Sair")
                break
            elif operacao_teoria=="11":
                print("##################################Métodos Especiais ou mágicos##################################")
                print("Os métodos especiais, também conhecidos como métodos mágicos, permitem definir comportamentos personalizados para classes em Python. Isso significa que você pode controlar como os objetos de uma classe se comportam em diferentes situações, como quando convertidos em string, utilizados em comparações ou em operações matemáticas.")
                print("Imagine um objeto que represente uma data. Você pode definir como ele se comporta quando convertido em string (formatação da data), quando utilizado em comparações (verificar se uma data é anterior ou posterior a outra) ou em cálculos matemáticos (adicionar ou subtrair dias).")
                print("Ao utilizar métodos mágicos, você torna seu código mais flexível, expressivo e fácil de manter, pois encapsula o comportamento específico de cada classe em um só lugar. Isso facilita a leitura e o entendimento do código, além de reduzir a necessidade de repetição.")
                print("##################################__init__(Construtor)##################################")
                print("O método __init__ é o alicerce da sua classe, pois é executado automaticamente na criação de cada objeto. Sua principal função é inicializar os atributos da instância, definindo seus valores iniciais e preparando-a para a ação.")
                continuar = input("Deseja printar o código abaixo? (s/n) ")
                if continuar.lower() == "s":
                    class ContaCorrente:
                    ...
                    print(conta1.nome_titular)  
                    print(conta1.saldo)      
                print("##################################str (Representação textual):##################################")
                print("O método __str__ define como um objeto será representado quando convertido para string, seja usando a função str() ou simplesmente imprimindo-o. Isso é crucial para garantir que seus objetos exibam informações úteis e relevantes.")
                continuar = input("Deseja printar o código abaixo? (s/n) ")
                if continuar.lower() == "s":
                    class ContaCorrente:
                        def __init__(self, nome_titular, saldo_inicial):
                        ...
                        def __str__(self):
                            return f"Conta Corrente: {self.nome_titular}, Saldo: R${self.saldo:.2f}"

                    conta1 = ContaCorrente("João Silva", 1000.00)
                    print(conta1) 
                print("##################################repr (Representação completa):##################################")
                print("Enquanto o  __str__ oferece uma visão geral amigável, o __repr__ fornece uma representação mais completa e precisa do objeto, útil para fins de depuração e análise.")
                continuar = input("Deseja printar o código abaixo? (s/n) ")
                if continuar.lower()=="s":
                    print("##################################Sem __repr__##################################")
                    class Retangulo:
                    ...
                    class Retangulo:
                        ...
                        def teste2(self):
                            print(f"Retangulo(base={self.base}, altura={self.altura})")
                    r = Retangulo(5, 3)
                    print(f"Representação: {r}")  
                    print(f"Área: {r.area()}")  
                print("##################################add: Somando Objetos##################################")
                continuar = input("Deseja printar o código abaixo? (s/n) ")
                if continuar.lower()=="s":
                     class Numero:
                     ...
                     resultado = num1.__add__(num2)
                     resultado = resultado.__add__(num3) 
                     ...
                     class Numero:
                        ...
                        def __add__(self, outro):
                            ...
                            if not isinstance(outro, Numero):
                             raise TypeError("Somente objetos Numero podem ser somados")
                            return Numero(self.valor + outro.valor)

                        def soma_numeros(numeros):
                        
                            if not isinstance(numeros, list):
                                raise TypeError("A função 'soma_numeros' espera uma lista de objetos Numero como argumento.")

                            if len(numeros) == 0:
                                raise ValueError("A lista de objetos Numero não pode estar vazia.")

                            soma = Numero(0)
                            for numero in numeros:
                                if not isinstance(numero, Numero):
                                 raise TypeError("A lista 'numeros' deve conter apenas objetos Numero.")
                            soma += numero

                            return soma

                        # Exemplo de uso
                        numeros = [Numero(1), Numero(2.5), Numero(-3), Numero(4)]
                        soma = soma_numeros(numeros)
                        print(f"A soma dos números na lista é: {soma.valor}")
                print("################################## __sub__: Subtrai Objetos##################################")
                continuar = input("Deseja printar o código abaixo? (s/n) ")
                if continuar.lower()=="s":
                    class Numero:
                        def __init__(self, valor):
                        ...
                        def __sub__(self, outro):
                            if not isinstance(outro, Numero):
                                raise TypeError("Somente objetos Numero podem ser subtraídos")
                            return Numero(self.valor - outro.valor)

                    num1 = Numero(5)
                    num2 = Numero(3)

                    resultado = num1.__sub__(num2)
                    print(f"A diferença entre num1 e num2 é: {resultado.valor}")
                print("################################## __mul__: multiplica Objetos##################################")
                continuar = input("Deseja printar o código abaixo? (s/n) ")
                if continuar.lower()=="s":
                    class Numero:
                        def __init__(self, valor):
                        ...
                        def __mul__(self, outro):
                            if isinstance(outro, Numero):
                                ...
                                raise TypeError("Multiplicação não suportada para este tipo")
                    ...
                    resultado_numeros = num1 * num2
                    print(f"Multiplicação de num1 e num2: {resultado_numeros.valor}")
                print("################################## __truediv__: Fração de Objetos##################################")
                continuar = input("Deseja printar o código abaixo? (s/n) ")
                if continuar.lower()=="s":
                    class Fração:
                        ...
                        def __truediv__(self, outro):
                            if not isinstance(outro, Fração):
                                raise TypeError("Divisão somente entre objetos Fração")
                            return Fração(self.numerador * outro.denominador, self.denominador * outro.numerador)

                    fração1 = Fração(10, 2)
                    fração2 = Fração(5, 3)

                    resultado = fração1 / fração2
                    print(f"O resultado de {fração1} dividido por {fração2} é: {resultado}")
                print("##################################len: Defini o tamanho dos objetos##################################")
                continuar = input("Deseja printar o código abaixo? (s/n) ")
                if continuar.lower()=="s":
                    class String:
